...
CREATE TABLE `mtx_branch` (
  ...
  `updated_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `FK_CAB0DCA8727ACA70` FOREIGN KEY (`parent_id`) REFERENCES `mtx_branch` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `mtx_comment` (
  ...
  PRIMARY KEY (`id`),  
  CONSTRAINT `FK_C10042846C8A81A9` FOREIGN KEY (`products_id`) REFERENCES `mtx_product` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
